bao <- c("shiny","shinydashboard","shinyjqui","ggplot2","data.table","stringr","dplyr","openxlsx","maptools","magrittr","readxl")
...
shinyApp(
  ui <- dashboardPage(title = "中国地图",skin = "green",
                      ...
                      sidebar = dashboardSidebar(width = 230,sidebarMenu(menuItem(text = "地图数据",icon = icon("bar-chart-o"),badgeLabel = "地图数据",badgeColor = "teal",tabName = "map"),
                                                                         ...
                                                                         menuItem(text = "纯色中国地图",icon = icon("map"),badgeLabel = "纯黑灰色",badgeColor = "black",tabName = "纯"),
  ...
  server <- function(input,output){
    ...
    output$合 <- renderTable({
      input$整合
      last <<- full_join(china_mapdata,data) %>% setDT() #合并地图数据和映射数据
      head(last,50)
    })
    ...
    output$纯分 <- renderPlot({
      part <- last[省市 %in% input$局部_order]
      input$纯分
