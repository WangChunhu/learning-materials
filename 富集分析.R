...
dff_extract <- function(data){
  ...
  result_rep <- fread(paste0(file.name,"-result_rep.csv")) %>% data.table()
  up <- result_rep[logFC > 1 & PValue < 0.05]
  up <- zhushi[up,on = "V1"]
  down <- result_rep[logFC < -1 & PValue < 0.05]
  down <- zhushi[down,on = "V1"]
  write.xlsx(x = up,file = paste0(file.name,"-up_rep.xlsx"))
  write.xlsx(x = down,file = paste0(file.name,"-down_rep.xlsx"))
  rm(y,et,countData)
  gc()
}
...
# countData <- data.matrix(T0_lf_1)
# #分组信息
# group_list <- factor(c(rep("Contral",2),rep("Treat",4)))
# #设置分组信息,并做TMM标准化
# y <- DGEList(counts = countData,group = group_list)
# # 计算样本内标准化因子
# y <- calcNormFactors(y) 
...
# topTags(et)
# #写出结果
# write.csv(topTags(et, n = nrow(y$counts)),paste0("result_rep.csv"), quote = FALSE)
# result_rep <- fread(paste0("result_rep.csv")) %>% data.table()
# up <- result_rep[logFC > 1 & PValue < 0.05]
# down <- result_rep[logFC < -1 & PValue < 0.05]
# write.xlsx(x = up,file = "up_rep.xlsx")
# write.xlsx(x = down,file = "down_rep.xlsx")

# #注释信息爬取cottonfgd
# require(Rcrawler)
# web.site <- paste0("https://cottonfgd.org/profiles/gene/",id,"/")
# gene_symbol <- function(url){
...
#   return(dan_sw_h)
# }
# a <- lapply(web.site, gene_symbol)
# df <- data.frame(matrix(unlist(a), nrow=length(a), byrow=T))
# setnames(df,c("X1","X2","X3","X4","X5"),c("gene_id","gene_name","注释","Swiss-Prot","NAU-trembl"))
# write.xlsx(x = df,file = "GH_注释.xlsx")
...
#读取WT_M.UP差异表达基因，将基因ID从GENE_SYMBOL转换为ENTREZ_ID：
setwd("I:/张振楠/转录组9-8")
WT_M.UP <- read.xlsx("WT_M_up.express.zhushi.xlsx") %>% data.table() %>% setnames("id","query")
WT_M.UP.atName <- gh.name[WT_M.UP,on = "query"]
data <- WT_M.UP.atName
fwrite(WT_M.UP.atName,"WT_M.UP.atName.csv")
WT_M.UP.atName[subject %in% NA] %>% nrow() ##无blast=67
WT_M.UP.atName[symbol %in% NA] %>% nrow() ##无名=244

#读取WT_M.down差异表达基因，将基因ID从GENE_SYMBOL转换为ENTREZ_ID：
WT_M.down <- read.xlsx("WT_M_down.express.zhushi.xlsx") %>% data.table() %>% setnames("id","query")
WT_M.down.atName <- gh.name[WT_M.down,on = "query"]
data <- WT_M.down.atName
fwrite(WT_M.down.atName,"WT_M.down.atName.csv")
WT_M.down.atName[subject %in% NA] %>% nrow() ##无blast=71
WT_M.down.atName[symbol %in% NA] %>% nrow() ##无名=332

#读取WT_M.UP差异表达基因，将基因ID从GENE_SYMBOL转换为ENTREZ_ID：
WT_P.UP <- read.xlsx("WT_p_up.express.zhushi.xlsx") %>% data.table() %>% setnames("id","query")
WT_P.UP.atName <- gh.name[WT_P.UP,on = "query"]
data <- WT_p.UP.atName
fwrite(WT_P.UP.atName,"WT_P.UP.atName.csv")
WT_p.UP.atName[subject %in% NA] %>% nrow() ##无blast=69
WT_p.UP.atName[symbol %in% NA] %>% nrow() ##无名=243

#读取WT_P.down差异表达基因，将基因ID从GENE_SYMBOL转换为ENTREZ_ID：
WT_P.down <- read.xlsx("WT_P_down.express.zhushi.xlsx") %>% data.table() %>% setnames("id","query")
WT_P.down.atName <- gh.name[WT_P.down,on = "query"]
data <- WT_P.down.atName
fwrite(WT_P.down.atName,"WT_P.down.atName.csv")
WT_P.down.atName[subject %in% NA] %>% nrow() ##无blast=80
WT_P.down.atName[symbol %in% NA] %>% nrow() ##无名=313

#down-down交集
WT_M.down.atName <- setnames(WT_M.down.atName,c("subject","symbol","query","WT1","WT2","M1","M2","gene_name","注释","logFC","logCPM","PValue","FDR"),c("subject_M","symbol_M","query","WT1_M","WT2_M","M1","M2","gene_name_M","注释_M","logFC_M","logCPM_M","PValue_M","FDR_M"))
nrow(WT_M.down.atName) #762
WT_P.down.atName <- setnames(WT_P.down.atName,c("subject","symbol","query","WT1","WT2","P1","P2","gene_name","注释","logFC","logCPM","PValue","FDR"),c("subject_P","symbol_P","query","WT1_P","WT2_P","P1","P2","gene_name_P","注释_P","logFC_P","logCPM_P","PValue_P","FDR_P"))
nrow(WT_P.down.atName) #756
jiaoji_down <- WT_P.down.atName[query %in% WT_M.down.atName$query]
nrow(jiaoji_down) #405
fwrite(jiaoji_down,"down-down.jiaoji.csv")

#up-up交集
WT_M.UP.atName <- setnames(WT_M.UP.atName,c("subject","symbol","query","WT1","WT2","M1","M2","gene_name","注释","logFC","logCPM","PValue","FDR"),c("subject_M","symbol_M","query","WT1_M","WT2_M","M1","M2","gene_name_M","注释_M","logFC_M","logCPM_M","PValue_M","FDR_M"))
nrow(WT_M.UP.atName) #530
WT_P.UP.atName <- setnames(WT_P.UP.atName,c("subject","symbol","query","WT1","WT2","P1","P2","gene_name","注释","logFC","logCPM","PValue","FDR"),c("subject_P","symbol_P","query","WT1_P","WT2_P","P1","P2","gene_name_P","注释_P","logFC_P","logCPM_P","PValue_P","FDR_P"))
nrow(WT_P.UP.atName) #497
jiaoji_up <- WT_P.UP.atName[query %in% WT_M.UP.atName$query]
nrow(jiaoji_up) #158
fwrite(jiaoji_up,"UP-UP.jiaoji.csv")
...
jiaoji_up <- fread("UP-UP.jiaoji.csv")
jiaoji_down <- fread("down-down.jiaoji.csv")
...
#GO/KEGG富集柱状图:
data.GO <- GO@result %>% data.table() %>% setorder(-"Count") %>% .[Count >= 8]
data.GO$p.adjust <- round(data.GO$p.adjust,digits = 3)
data.GO <- setorder(data.GO,"ONTOLOGY",-"Count")
...
# pdf(file = "up-up.pdf",width = 25,height = 15)
# barplot(GO, split="ONTOLOGY")+ #柱状图
...
#GO图
pdf("I:/张振楠/转录组9-8/富集分析/作图/二次/图/UP.GO富集.pdf",width = 22,height = 12) 
ggplot(data = data.GO,aes(Description,Count))+
  geom_col(aes(fill = ONTOLOGY),width = 0.7)+
  geom_point(aes(color = p.adjust,size = 10))+
...
ggplot(data = data.GO,aes(Description,Count,shape = ONTOLOGY))+ #,color = p.adjust
  geom_point(aes(color = p.adjust,size = Count))+
  scale_color_gradient(low = "red",high = "green",limits=c(0,0.001),breaks=c(0,0.001))+
  theme_classic()+
  scale_size(limits = c(5,15),breaks = c(5,10,15))+
  coord_flip()+
  scale_y_continuous(limits = c(8,14),breaks = c(8,10,12,14))+
  theme(axis.text = element_text(size = 20),axis.title = element_text(size = 25),legend.text = element_text(size = 20),legend.title = element_text(size = 25),strip.text = element_text(size = 20))+
  guides(shape = guide_legend(override.aes = list(size = 5)))
dev.off()

#KEGG图
data.KEGG <- KEGG@result %>% data.table() %>% setorder(-"Count") %>% .[Count >= 10]
data.KEGG$p.adjust <- round(data.KEGG$p.adjust,digits = 3)
data.KEGG$ONTOLOGY <- factor(data.KEGG$ONTOLOGY,levels = unique(data.KEGG$ONTOLOGY))
data.KEGG$Description <- factor(data.KEGG$Description,levels = rev(unique(data.KEGG$Description)))
pdf("I:/张振楠/转录组9-8/富集分析/作图/二次/图/DOWN.KEGG富集.pdf",width = 15,height = 3)
ggplot(data = data.KEGG,aes(Description,Count))+
  geom_col(aes(fill = p.adjust),width = .15)+
  scale_fill_gradient(low = "red",high = "blue",limits=c(0,0.001),breaks=c(0,0.001))+
  theme_classic()+
  coord_flip()+
  theme(axis.text = element_text(size = 20),axis.title = element_text(size = 25),legend.text = element_text(size = 20),legend.title = element_text(size = 25),strip.text = element_text(size = 20))
...
ggplot(data = data.KEGG,aes(Description,Count))+
  geom_point(aes(color = p.adjust,size = Count))+
  scale_color_gradient(low = "red",high = "blue",limits=c(0,0.003),breaks=c(0,0.001,0.002))+
  theme_classic()+
  coord_flip()+
  scale_y_continuous(limits = c(10,12),breaks = c(10,11,12))+
  theme(axis.text = element_text(size = 25),axis.title = element_text(size = 30),legend.text = element_text(size = 25),legend.title = element_text(size = 30))
dev.off()
...
###自定义函数
vol <- function(data){
  data$threshold[data$PValue < 0.05 & data$logFC > 1] = "Up"
  data$threshold[data$PValue < 0.05 & data$logFC < -1] = "Down"
  data$threshold[data$PValue >= 0.05 | (data$logFC >= -1 & data$logFC <= 1)] = "Non"
  p <-
    ggplot(data,aes(x=logFC,y=-log10(PValue),colour=threshold))+xlab("log2(Fold Change)")+ylab("-log10(PValue)")+
...
pdf("I:/张振楠/转录组9-8/富集分析/作图/二次/图/火山图.pdf",width = 17,height = 15)
vol(data = WT_P)
vol(data = WT_M)
dev.off()
...
##韦恩图
pdf("I:/张振楠/转录组9-8/富集分析/作图/二次/图/韦恩图.pdf",width = 10,height = 10)
###总
...
fwrite(dff.inter.FPKM.zhushi,"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/dff.inter.FPKM.zhushi.csv")
...
fwrite(up.inter.FPKM.zhushi,"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/up.inter.FPKM.zhushi.csv")
...
fwrite(down.inter.FPKM.zhushi,"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/down.inter.FPKM.zhushi.csv")
...
fwrite(jiaoji.kegg2[[1]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.kegg2.row1.csv")
fwrite(jiaoji.kegg2[[2]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.kegg2.row2.csv")
jiaoji.up.kegg2 <- kegg2GH(data.kegg = data.kegg)
fwrite(jiaoji.up.kegg2[[1]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.up.kegg2.row1.csv")
fwrite(jiaoji.up.kegg2[[2]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.up.kegg2.row2.csv")
jiaoji.down.kegg2 <- kegg2GH(data.kegg = data.kegg)
fwrite(jiaoji.down.kegg2[[1]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.down.kegg2.row1.csv")
fwrite(jiaoji.down.kegg2[[2]],"I:/张振楠/转录组9-8/富集分析/作图/二次/作图数据/jiaoji.down.kegg2.row2.csv")
