...
gstat.idw <- R6Class(classname = "gstat.idw", 
                     public = list(),
                     private = list(
                       # 私有成员
                       # 插值idp验证函数 ------------------------------------------------------------------
                       idw.RMSE.huhu <- function(interpolation.colName = interpolation.colName, data = data, lonlat.colNum = lonlat.colNum, map.path = map.path, idp = idp, write.dir = write.dir, write.name = write.name){
                         #' @description 使用宽数据
                         ...
                         # 插值数据
                         idw.data <- data %>% na.omit() %>% data.table()
                       ...
                       # 插值自定函数 ------------------------------------------------------------------
                       idw.huhu <- function(interpolation.colName = interpolation.colName, data = data, Group = NULL, lonlat.colNum = lonlat.colNum, map.path = map.path, grid.n = grid.n, idp = idp, write.dir = write.dir, write.name = write.name){
                         #' @description 使用长数据
                         #' @description 根据站点bio信息插值到地图上
                         #' @param interpolation.colName 插值的值列名
                         #' @param data 带有站点、经纬度及插值信息的数据框(xlsx)
                         #' @param Group 如果有多个分组则指定插值分组,并必须使分组列名为group
                         #' @param lonlat.colNum 经纬度数据框，经度在前
                         #' @param map.path 地图shp地址
                         ...
                         if (is.null(Group)) {
                           # 读取带有站点、经纬度及插值信息的数据框并转为data.table
                           idw.data <- data %>% na.omit() %>% data.table()
                         }else{
                           idw.data <- data %>% na.omit() %>% data.table() %>% .[group %in% Group]
                         }
                         ...
                         grd <- as.data.frame(spsample(x = p, type = "regular", n = grid.n))
                         names(grd) <- c("X", "Y")
                         ...
                         # 保存栅格
                         writeRaster(x = r.m, filename = paste0(write.dir, write.name, "_", interpolation.colName, "-idp.", idp, "-RMSE.", round(RMSE, 3), ".tif"), overwrite=TRUE)
                       },
